...
**Aesthetic fixes found in the pass.** Collection heading no longer wraps (link moved to the eyebrow row); long product names wrap instead of truncating, with price + button pinned so rows align; hero secondary link hidden on phones where it wrapped; bridal photo 4:3 on phones (was a full screen); widow on "Real hands."; tablet gets 3 product columns (6 designs), a 2×2 trust row and a smaller H1; section rhythm alternates.

**Hero vs collection (Gestalt, proximity / common region).** Hero and collection originally shared the bone background with only 45 px between the hero CTA and the collection label, against 16–24 px gaps inside the hero — under a 2× ratio, so they read as one group. Adding space would push products out of the first screen, so the collection got its own band instead (`bg-shell` + hairline top border). Background rhythm now: bone (hero) → shell (collection) → paper (fit) → bone (why press-ons, bordered cards) → champagne (bridal) → paper (customers) → shell (trust) → bone (FAQ). The collection band still starts inside the first phone screen (564 px on a 390×844 viewport).
