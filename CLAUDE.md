...
- **Bridal hero** shows price, "Add Trio to bag" (slug `bridal-trio-classic`) and "Order at least 4 weeks before your Mehendi" in the first screen. **About hero** uses `public/images/about-set-card-*` (finished set on the brand card) so it doesn't repeat the home hero photo; story section has `id="story"`.
