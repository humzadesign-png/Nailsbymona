...
- **Live WhatsApp number is a placeholder (`+923000000000` in StoreSettings, found 2026-09-24)** — every wa.me link goes nowhere until the real number is entered in Admin → Settings.
- **Home page redesign is LIVE (2026-09-24).** Research + rationale: `docs/ux/home-redesign-2026-09.md`; reference prototype: `html/home-v2.html` (`html-mockups` launch config → http://localhost:8081/home-v2.html). Why: ad visitors (≈60% of traffic, ~80% in Instagram's in-app browser) spent ~6 s on home and scrolled ~12%; the first product/price sat 4 screens down on an 18.5-screen page. Now 8.4 screens, collection band (`bg-shell`) starts inside the first phone screen. Route `/` passes `$homeProducts` (8, `is_featured` first), `$productCount`, `$minPrice`. No announcement bar (deliberate — see doc §5b). Only real proof: no invented reviews/counts; cards use the shop's tier badges.
- **Shared hero: `resources/views/components/page-hero.blade.php`** — used by Home, Bridal and About. Photo never covered, 190 px tall on phones, 50/50 split from `md`. Props: `image` (base name in `public/images`, needs `-{w}.webp` + `.jpg` per width), `widths`, `alt`, `badge`, `position`, `bg`. Use it for any new top-level page hero instead of a full-bleed/frosted-card hero. **Don't write `<x-page-hero>` inside a Blade comment** — component tags are compiled before comments are stripped.
- **Bridal hero** shows price, "Add Trio to bag" (slug `bridal-trio-classic`) and "Order at least 4 weeks before your Mehendi" in the first screen. **About hero** uses `public/images/about-hero-{768,960}` (polka-dot/floral set around a coffee cup, supplied by Humza — only 960 px wide, don't upscale) so it doesn't repeat the home hero photo; story section has `id="story"`. About's process timeline uses the brand icons (camera, palette, gift box, heart) + a brush and check-magnifier in the same 24px/1.5-stroke style, in 48px paper/hairline circles like the home step markers — no WhatsApp promises in its copy (updates are by email).
- **`/shop?filter=everyday|signature|glam|bridal|under3000`** pre-selects a filter pill (home shortcuts link here). `under3000` filters on `data-price`.
